...
| [go-to-market-audit](./skills/marketing/go-to-market-audit/SKILL.md) | 生の製品・マネタイズのアイデアを、検証済みの GTM 計画に鍛え上げます:既存アセットのエビデンス調査、12 ステーションの意思決定ツリー尋問(戦略的役割・ICP・ビーチヘッド市場・価格設定・構築順序・撤退基準など)を経て、創業ドキュメント GTM-STRATEGY.md とサインオフチェックリスト付きの洗練された自己完結型 HTML レビューを出力。Express モードは一括実行で全決定を記録、対話モードは一問ずつ尋問します。 |
